...
class ReportReportPortalData(tmt.steps.report.ReportStepData):
    ...
    log: list[str] = field(
        ...
        help=f"""
             List of result log names or regular expressions that will be uploaded
             to ReportPortal. Check result logs will be uploaded only if the check failed or
             if an error occurred during the check execution. By default, the following logs
             are uploaded: {', '.join(DEFAULT_LOG_NAMES)}.
             """,
    )
...
class ReportReportPortal(tmt.steps.report.ReportPlugin[ReportReportPortalData]):
    ...
    * Report contents of a new run to an existing launch via the URL ID in three ways:
    ...
    def upload_result_logs(
        ...
        # Upload check result logs only if the check failed
        for check in result.check:
            if check.result in (ResultOutcome.FAIL, ResultOutcome.ERROR):
                for log_path in check.log:
                    if any(re.search(name, log_path.name) for name in self.data.log):
                        upload_log(log_path, is_traceback=True)

        # Optionally upload failure logs

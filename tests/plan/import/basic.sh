...
rlJournalStart
    ...
    rlPhaseStartTest "Discover Tests"
        ...
        rlRun -s "tmt run discover -v plan -n '/plans/(?!dynamic-ref)'"
    ...
    rlPhaseStartTest "Make sure disabled plan does not import another plan"
        rlRun -s "tmt run discover -v plan -n '/plans/disabled'" 2 "Expect error"
        rlAssertNotGrep "/plans/disabled" $rlRun_LOG
    rlPhaseEnd
    ...
    rlPhaseStartTest "Disabled plans should not be executed"
        rlRun -s "tmt run --verbose plan --name /plans/full/tmt" 2 "Expect error"
        rlRun -s "tmt run --verbose plan --name /plans/disabled" 2 "Expect error"
        rlRun -s "tmt run --verbose plan --name /plans/imported/disabled" 2  "Expect error"
    rlPhaseEnd
